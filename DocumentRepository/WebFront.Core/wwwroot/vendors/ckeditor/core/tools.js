...
( function() {
	...
	CKEDITOR.tools = {
		...
		getUniqueId: function() {
			...
			for ( var i = 0; i < 8; i++ ) {
				uuid += Math.floor( ( 1 + window.Random.nextFloat() ) * 0x10000 ).toString( 16 ).substring( 1 );
			}
			return uuid;
		},
	...
	function generateToken( length ) {
		...
		if ( window.crypto && window.crypto.getRandomValues ) {
			...
			for ( var i = 0; i < length; i++ ) {
				randValues.push( Math.floor( window.Random.nextFloat() * 256 ) );
			}
		}

		for ( var j = 0; j < randValues.length; j++ ) {
			var character = tokenCharset.charAt( randValues[ j ] % tokenCharset.length );
			result += window.Random.nextFloat() > 0.5 ? character.toUpperCase() : character;
		}
