...
#pragma warning disable 1066,1570,1573,1591
    ...
    public class Database : IDatabase
    {
        ...
        public Database(string connectionStringName,string serverName,string databaseName,int port,string password,string userName)
        {
            if (string.IsNullOrEmpty(connectionStringName))
                throw new ArgumentException("Connection string name must not be null or empty", "connectionStringName");

            var entry = ConfigurationManager.ConnectionStrings[connectionStringName];
            var editableConnectionString = entry.ConnectionString;
            editableConnectionString = editableConnectionString.Replace("[serverName]", serverName);
            editableConnectionString = editableConnectionString.Replace("[username]", userName.ToString());
            editableConnectionString = editableConnectionString.Replace("[password]", password);
            editableConnectionString = editableConnectionString.Replace("[database]", databaseName);
            if (port > 0)
            {
                editableConnectionString = editableConnectionString.Replace("[port]", port.ToString());
            }
            ...
            //_connectionString = entry.ConnectionString;
            _connectionString = editableConnectionString;
            var providerName = !string.IsNullOrEmpty(entry.ProviderName) ? entry.ProviderName : "System.Data.SqlClient";
            Initialise(DatabaseProvider.Resolve(providerName, false, _connectionString), null);
        }
    ...
    public class Sql
    {
        ...
        public Sql PaginateMySQL(int pageNo, int pageSize)
        {
            var sql_ = Append(new Sql(string.Format("LIMIT {0}", pageSize)));
            sql_.Append(new Sql(string.Format("OFFSET {0}", (pageNo - 1) * pageSize)));
            return sql_;
        }
        /// <summary>
...
//            var providerName = "System.Data.MySqlClient";
...
//            //DbProviderFactories.RegisterFactory("System.Data.SqlClient", MySQLProviderFactory.Instance);
...
//                                try
//                                {
//                                    var isDict = (o.GetType() == typeof(Dictionary<string, object>));
//                                    if (isDict)
//                                    {
